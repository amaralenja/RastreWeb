...
import { Globe, Plus, Copy, Check, Code, Trash2, ShieldCheck, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-100">Sites & Projetos Cadastrados</h2>
        ...
        {projects && projects.length > 0 ? (
          projects.map((proj) => {
            ...
            return (
              <div
                key={proj.id}
                className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between space-y-4 hover:border-slate-700 transition-colors"
              >
                <div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-slate-100 text-base">{proj.name}</h3>
                        <p className="text-xs text-slate-400 font-mono">{proj.domain || 'Dominio não especificado'}</p>
                      </div>
                    </div>
                  ...
                  {/* Site Key Badge */}
                  <div className="mt-4 p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between text-xs font-mono">
                    <span className="text-slate-400 truncate">Site Key: <strong className="text-slate-200">{proj.site_key}</strong></span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-950 p-3 rounded-xl border border-slate-800/80 font-mono text-[11px] text-slate-300 overflow-x-auto break-all select-all">
                    {snippetText}
                  </div>
                </div>
              </div>
          ...
          <div className="col-span-full bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
            ...
            <h3 className="text-base font-semibold text-slate-200">Nenhum site cadastrado ainda</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-slate-100 mb-1">Cadastrar Novo Site</h3>
            <p className="text-xs text-slate-400 mb-4">Informe o nome do site e o domínio onde o script será instalado.</p>

            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Nome do Projeto</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Domínio do Site</label>
                <input
                  type="text"
