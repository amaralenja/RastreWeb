...
import { Play, Filter, AlertCircle, Monitor, Smartphone, Tablet, Search, Clock, ExternalLink } from 'lucide-react';

export default function Sessions({ sessions, onPlaySession }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-100">Gravações de Sessão (Replays)</h2>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <label className="flex items-center gap-2 cursor-pointer bg-slate-950 px-3 py-2 rounded-xl border border-slate-800 text-xs text-slate-300 select-none">
            <input
              type="checkbox"
              checked={rageOnly}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-950/80 text-slate-400 font-medium uppercase tracking-wider border-b border-slate-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-200">
              {filteredSessions.length > 0 ? (
                filteredSessions.map((s) => (
                  <tr key={s.id || s.session_id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="py-3.5 px-4 font-mono font-medium text-blue-400">
                      {s.session_id.substring(0, 14)}...
                    </td>
                    <td className="py-3.5 px-4 max-w-xs truncate font-medium text-slate-200">
                      {s.page_entry || '/'}
                    </td>
                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-slate-950 rounded-lg border border-slate-800 capitalize text-slate-300">
                    ...
                    <td className="py-3.5 px-4">
                      {s.rage_click ? (
                        <span className="px-2 py-0.5 text-[11px] font-medium bg-red-500/10 text-red-400 border border-red-500/20 rounded-full inline-flex items-center gap-1">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium inline-flex items-center gap-1.5 transition-colors shadow-md shadow-blue-600/20"
                      >
